...
export function MyStreamsPage() {
  ...
  const handleEditStream = async (streamId: number, title: string, description: string) => {
    try {
      ...
      await streamService.updateStream(streamId, { title, description }, authHeader)
      
      // Invalidate and refetch the streams data
      queryClient.invalidateQueries({ queryKey: ['my-streams'] })
      
      toast.success('Cập nhật stream thành công!')
    } catch (error) {
  ...
  if (isLoading) {
    return (
      <div className="container mx-auto px-4 py-8">
        <h1 className="text-3xl font-bold text-gray-900 mb-8">Streams của tôi</h1>
        ...
        <div className="flex justify-center items-center min-h-[200px]">
          <div className="text-center">
            <p className="text-red-600 mb-4">
              Không thể tải danh sách streams: {error?.message || 'Lỗi không xác định'}
            </p>
  ...
  return (
    <div className="container mx-auto px-4 py-8">      <div className="flex justify-between items-center mb-8">
        <h1 className="text-3xl font-bold text-gray-900">Streams của tôi</h1>
      ...
      {allStreams.length === 0 ? (
        <div className="flex justify-center items-center min-h-[200px]">          <div className="text-center">
            <p className="text-gray-600 text-lg mb-4">Bạn chưa có stream nào.</p>
            <Button onClick={() => navigate({ to: '/stream/create' })}>
              Tạo stream đầu tiên
            </Button>
          </div>
        ...
        <>
          {/* Streams Grid */}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">            {allStreams.map((stream) => (
              <StreamCardWithActions
                ...
                onEdit={(title, description) => handleEditStream(stream.streamId, title, description)}
